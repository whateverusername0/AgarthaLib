...
namespace AgarthaLib.MonoBehavior
{
    ...
    public abstract class AgarthanSingleton<T> : AgarthanBehaviour where T : AgarthanSingleton<T>
    {
        ...
        public static T Instance
        {
            get
            {
                _instance = _instance == null ? FindObjectsByType<T>(FindObjectsInactive.Include, FindObjectsSortMode.InstanceID).FirstOrDefault() : _instance;
                _instance = _instance == null ? new GameObject() { name = $"[Singleton] {typeof(T).Name}", hideFlags = HideFlags.DontSave }.EnsureComponent<T>() : _instance;
                return _instance;
            }
        }
